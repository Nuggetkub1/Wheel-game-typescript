...
const Wheel: React.FC = () => {
  ...
  const spinWheel = (): void => {
    ...
    setTimeout(() => { // set เวลาการหมุน
      const prizeIndex: number = getWeightedRandomIndex(segments.map((seg) => seg.probability)); 
      ...
      const randomSpin = Math.floor(minSpin + Math.random() * (maxSpin - minSpin));// สมการคำนวนองศาการหมุน
  ...
  const buyCoinsWithMod = () => { // การเปลี่ยนจาก Mod เป็น coin
    if (mod >= 500) {
  ...
  const getWeightedRandomIndex = (weights: number[]): number => { // เลือกส่วนที่ชนะจากความน่าจะเป็น
    ...
    return cumulativeWeights.findIndex((cumulativeWeight) => randomValue <= cumulativeWeight);
  };

  const calculateArcPath = ( //คำนวนเส้นโค้งที่ใช้ในการหมุนตอนเริ่่ม และตอนท้าย
    index: number,
  ...
  return ( // ส่วนประกอบในหน้า web และ ปุ่มต่าง ๆ
